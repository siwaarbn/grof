...
if [ -f "$T1_SCRIPT" ]; then
  ...
  sudo /usr/bin/python3 "$T1_SCRIPT" --output "$OUTPUT_DIR/cpu_correlation_with_stack.json" &
  T1_PID=$!
  sleep 5  # wait for BPF to compile and attach uprobes
else
...
if [ -f "$T2_LIB" ]; then
  GROF_OUTPUT_DIR="$OUTPUT_DIR" LD_PRELOAD="/usr/local/cuda-12.8/targets/x86_64-linux/lib/libcupti.so.12 $T2_LIB" /mnt/data/venvs/grof-env/bin/python3 "$WORKLOAD" &
  WORKLOAD_PID=$!
  export GROF_TARGET_PID=$WORKLOAD_PID
  wait $WORKLOAD_PID
else
